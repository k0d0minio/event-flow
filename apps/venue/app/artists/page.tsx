...
export default async function ArtistsPage() {
  ...
  const { data: artists, error } = await supabase
    .from("artists")
    .select("id, display_name, genre, bio")
  ...
  // Fetch profiles to get artist_data for stage names
  ...
  const profilesMap = new Map(profiles?.map((p) => [p.id, p.artist_data as Record<string, unknown>]) || [])
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {artists && artists.length > 0 ? (
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {artists.map((artist) => {
              const artistData = profilesMap.get(artist.id) || {}
              const stageName = (artistData.stage_name as string) || artist.display_name || "Unknown Artist"
              const primaryGenre = (artistData.primary_genre as string) || artist.genre || "Unknown Genre"
              const bioShort = (artistData.bio_short as string) || artist.bio || ""
