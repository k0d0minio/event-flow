 ...
 * 
 * Usage:
 *   SUPABASE_URL=<url> SUPABASE_SERVICE_ROLE_KEY=<key> pnpm tsx scripts/create_admin_users.ts
 */
  ...
  for (const { email, password } of ADMIN_USERS) {
    try {
      ...
      if (existingUser) {
        ...
        // Create profile with admin role (the trigger will create a profile with 'artist' role, so we need to update it)
        const { error: profileError } = await supabaseAdmin
          .from('profiles')
          .upsert({
            id: newUser.user.id,
